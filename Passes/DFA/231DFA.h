 ...
 */
    ...
    private:
        ...
        void initializeBackwardMap(Function *func) {
            ...
            for (Function::iterator bi = func->begin(), e = func->end(); bi != e; ++bi) {
                ...
                if (dyn_cast<ReturnInst>(&bi->back())) {
                    addEdge(nullptr, &bi->back(), &InitialState);
                }
            }
        }
    ...
    public:
        ...
        void runWorklistAlgorithm(Function *func) {
            ...
            //std::cout << "finished initialization of worklist" << std::endl;
            for (auto& block : *func) {
                auto& first = block.front();
                if (isa<PHINode>(first)) {
                    worklist.emplace_back(InstrToIndex[&first]);
                }
                for (auto& instr : block) {
                    if (!isa<PHINode>(instr)) {
                        worklist.emplace_back(InstrToIndex[&instr]);
                    }
                }
            }

            //std::cout << "finished stage2" << std::endl;
            // (3) Compute until the work list is empty
            while (!worklist.empty()) {
                auto instr_id = worklist.front();
                //std::cout << "processing instruction:" << instr_id << std::endl;
                worklist.pop_front();
                std::vector<unsigned> incoming, outgoing;
                std::vector<Info*> infos;
                getIncomingEdges(instr_id, &incoming);
                getOutgoingEdges(instr_id, &outgoing);
                /* std::cout << "incoming nodes:";
                std::copy(incoming.begin(), incoming.end(), std::ostream_iterator<unsigned>(std::cout, " "));
                std::cout << "outgoing nodes:";
                std::copy(outgoing.begin(), outgoing.end(), std::ostream_iterator<unsigned>(std::cout, " ")); */
                flowfunction(IndexToInstr[instr_id], incoming, outgoing, infos);
                assert(infos.size() == outgoing.size());
                for (unsigned i = 0; i < outgoing.size(); i ++) {
                    auto& old_info = EdgeToInfo[std::make_pair(instr_id, outgoing[i])];
                    Info joined;
                    Info::join(old_info, infos[i], &joined);
                    if (!Info::equals(old_info, &joined)) {
                        old_info = new Info(joined);
                        worklist.emplace_back(outgoing[i]);
                    }
                }
            }
        }
